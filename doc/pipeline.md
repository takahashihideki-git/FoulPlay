...
Pipeline を前提にしたスクリプトは、作業のコンテキストのことは度外視して、UNIX のコマンドのような"単一責任の原則"の下で、まさに目の前に与えられたテキストの書き換えのみに専念すればよいことになります。
...
ちなみに、Selector 欄で選択されたファイル、Filters 欄で指定されたスクリプトファイルの内容は、変換作業実行時に読み込まれるので、フォルダを選択した後で、それらのファイルの内容を書き換えたとしても、Pipeline をロードし直したり、「Refresh」ボタンをクリックする必要はありません。
