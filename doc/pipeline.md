...
Filter 欄から Selector 欄にファイルをドラッグしてスクリプトファイルの指定を取り消すこともできます。
...
Selector 欄でファイルが選択され、Filter 欄にスクリプトファイルが配置されると、「Run」ボタンがアクティブになります。
「Run」をクリックすると、Selector、Filter、Terminator 欄で決定されたシナリオにしたがって、変換作業が実行されます。
